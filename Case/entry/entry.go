...
func EntryUpdate(c *fiber.Ctx) error {
	...
	entry.Update_Date = time.Now().String()
...
func EntryCreate(c *fiber.Ctx) error {
	...
	if entry.Tag == "1000" || entry.Tag == "1001" || entry.Tag == "1002" {
		...
		//
		entry.Create_Date = time.Now().String()
		entry.Update_Date = time.Now().String()
...
func EntryAllGet(c *fiber.Ctx) error {
	...
	for rows.Next() {
		rows.Scan(&entryaa.Id, &entryaa.Entry_Code, &entryaa.Entry_Title, &entryaa.Create_Date, &entryaa.Update_Date, &entryaa.Status, &entryaa.Assig, &entry.Tag, &entryaa.EntryRelation)
...
func EntryTopAllGet(c *fiber.Ctx) error {
	...
	rows, _ := db.Query("SELECT * FROM entry WHERE entry_Code=$1", entry.Entry_Code)
	...
	rows2, _ := db.Query("SELECT * FROM entryrelation WHERE main_Entry=$1", entry.Entry_Code)
	...
	rows3, _ := db.Query("SELECT * FROM user WHERE id=$1", entry.Assig)

	for rows3.Next() {
		rows3.Scan(&entry.User.Id, &entry.User.User_Name, &entry.User.Name, &entry.User.Surname, &entry.User.Is_Active)

	}
	rows4, _ := db.Query("SELECT * FROM tagpro WHERE tag_id=$1", entry.Tag)
	...
	rowss, _ := db.Query("SELECT * FROM comments WHERE entry_id=$1", entry.Entry_Code)
